...
export const timeFix = (): string => {
  ...
  return hour < 9 ? '早上好呀' : hour <= 11 ? '上午好呀' : hour <= 13 ? '中午好呀' : hour < 20 ? '下午好' : '晚上好呀'
}
...
export const openWindow = (url: string, title: string, w: number, h: number) => {
  ...
  const dualScreenLeft = window.screenLeft !== undefined ? window.screenLeft : screen.height
  // eslint-disable-next-line no-restricted-globals
  const dualScreenTop = window.screenTop !== undefined ? window.screenTop : screen.width
  // eslint-disable-next-line no-restricted-globals
